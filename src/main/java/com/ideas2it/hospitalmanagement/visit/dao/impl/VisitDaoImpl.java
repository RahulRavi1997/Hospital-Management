...
public class VisitDaoImpl extends GenericDao implements VisitDao {
	...
	String VISIT_BY_PHYSICIANID ="from Visit where PHYSICIAN";
	String IDS = "ids";
	...
	public List<Visit> retrieveByPhysicianId(Integer physicianId) throws ApplicationException {
		try {
			return super.getAllByAttribute(Visit.class , "PHYSICIAN" , physicianId);
		} catch (ApplicationException e) {
			Logger.error(String.format(Constants.VISIT_SEARCH_EXCEPTION, physicianId), e);
			throw new ApplicationException(String.format(Constants.VISIT_SEARCH_EXCEPTION,
					physicianId));
		}
	}
