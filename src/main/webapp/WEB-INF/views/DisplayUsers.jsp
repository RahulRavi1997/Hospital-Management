...
<html>
  ...
  <jsp:include page="header.jsp"/>
    ...
    <c:if test="${empty users}">
      <h2 align="center">No Users are available/present at the moment. Sorry!</h2>
    </c:if>
            ...
            <c:forEach var="user" items="${users}">
              ...
              <c:if test="${user.isActive()}">
                  ...
                  <form action ="deleteUser" method="POST">
                    <input type="hidden" name="id" value="${user.id}"/>
                    <td class="type"><button type="submit"  class="btn-danger" onclick="return confirm('Delete user id : ${user.id} ?')" >Delete </button></td>
                  </form>
                </c:if>
          </tr>
          </c:forEach>
